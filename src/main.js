...
function subscribeToEvents() {
  ...
  modules.forEach(({ contract, events }) => {
    try {
      const moduleConfig = MODULES[contract];
      const address = MODULE_ADDRESS_BOOK[DEFAULT_NETWORK]?.[contract] || moduleConfig?.address;
      const abi = moduleConfig?.abi;
      if (!address || !abi) return;
      const wsContract = new ethers.Contract(address, abi, wsProvider);
      events.forEach((eventName) => {
        const eventExists = Array.isArray(abi)
          ? abi.some((fragment) => fragment?.type === "event" && fragment?.name === eventName)
          : false;
        if (!eventExists) {
          console.warn(`Event ${eventName} not found in ${contract} ABI. Skipping subscription.`);
          return;
        }
